...
const FaceAuthRegisterOwn=props=>{
    ...
    function dataURLtoFile(dataurl, filename) {
        ...
        const registerClick=()=>{
			...
			if(facesDetected===1){
                ...
                axios.post(`${FACE_AUTH_URL}/create/${userPhone}`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                }).then(res=>{
                    setErrorMessages([`Face registred successfully...`])
                }).catch(err=>{
...
export default FaceAuthRegisterOwn